...
The user flow has 3 steps: **1 · Load data → 2 · Select SKU → 3 · Review & approve**. Users should never lose context: anything they need to check (a rule, a competitor's listing) opens in a drawer over the current screen instead of navigating away.

In THIS step, build the upload flow, the SKU picker, the report screen, the rule drawer, the guidelines page and a deterministic rules engine. NO AI/LLM calls yet — those come in a later step.
...
- Routes: "/" = Load data, "/skus" = Select SKU, "/skus/:skuId" = report, "/guidelines" = guidelines page.
- Top nav: "Ally · Competitor Content Intelligence" on the left; "1 · Load data", "2 · Select SKU", "Guidelines" on the right. "2 · Select SKU" is disabled until data is loaded. "Guidelines" opens the guidelines drawer.
- A simple 3-step progress indicator (Load data → Select SKU → Review & approve) at the top of the Load data, Select SKU and report screens, highlighting the current step.
...
## Screen: Load data ("/", the landing page)
...
- A drag-and-drop zone plus a "Choose file" button. Show a clear drag-over state and a parsing spinner.
